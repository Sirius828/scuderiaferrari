...
static void RecordSkippedBox(ppocr_text_recog_array_result_t* result,
                             ...
                             const char* reason)
{
    ...
    ++result->skipped_box_count;
    if (result->skipped_box_summary[0] != '\0') {
        std::strncat(result->skipped_box_summary, "; ",
                     sizeof(result->skipped_box_summary) - std::strlen(result->skipped_box_summary) - 1);
    }
    char item[192];
    std::snprintf(item, sizeof(item), "box=(%d,%d)-(%d,%d) reason=%s",
                  box[0], box[1], box[4], box[5], reason == nullptr ? "invalid" : reason);
    std::strncat(result->skipped_box_summary, item,
                 sizeof(result->skipped_box_summary) - std::strlen(result->skipped_box_summary) - 1);
}

static bool GetRotateCropImage(const cv::Mat& srcimage,
                               const std::array<int, 8>& box,
                               cv::Mat* dst_img,
                               std::string* reason)
{
    if (dst_img == nullptr || srcimage.empty() || srcimage.cols <= 0 || srcimage.rows <= 0) {
        if (reason != nullptr) *reason = "empty_source";
        return false;
    }
    ...
    points.reserve(4);
    for (int i = 0; i < 4; ++i) {
        int x = std::clamp(box[2 * i], 0, srcimage.cols - 1);
        int y = std::clamp(box[2 * i + 1], 0, srcimage.rows - 1);
        points.push_back({x, y});
    }

    int x_collect[4] = {box[0], box[2], box[4], box[6]};
    int y_collect[4] = {box[1], box[3], box[5], box[7]};
    int left = std::clamp(int(*std::min_element(x_collect, x_collect + 4)), 0, srcimage.cols - 1);
    int right = std::clamp(int(*std::max_element(x_collect, x_collect + 4)), 0, srcimage.cols - 1);
    int top = std::clamp(int(*std::min_element(y_collect, y_collect + 4)), 0, srcimage.rows - 1);
    int bottom = std::clamp(int(*std::max_element(y_collect, y_collect + 4)), 0, srcimage.rows - 1);

    if (right <= left || bottom <= top) {
        if (reason != nullptr) *reason = "empty_bounds";
        return false;
    }
    ...
    if (img_crop.empty()) {
        if (reason != nullptr) *reason = "empty_crop";
        return false;
    }
    ...
    if (img_crop_width <= 0 || img_crop_height <= 0) {
        if (reason != nullptr) *reason = "zero_perspective_size";
        return false;
    }
    ...
    if (dst_img->empty()) {
        if (reason != nullptr) *reason = "empty_perspective_crop";
        return false;
    }
...
int inference_ppocr_system_model(ppocr_system_app_context* sys_app_ctx, image_buffer_t* src_img, ppocr_det_postprocess_params* params, ppocr_text_recog_array_result_t* out_result)
{
    int ret;
    // Detect Text
    if (out_result == nullptr || src_img == nullptr || src_img->virt_addr == nullptr ||
    ...
    for (int i=0; i < boxes_result.size(); i++) {
        ...
        if (!GetRotateCropImage(in_image, boxes_result[i], &crop_image, &skip_reason)) {
            RecordSkippedBox(out_result, boxes_result[i], skip_reason.c_str());
            continue;
        }
